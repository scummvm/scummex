...
const char *DeScumm::get_num_string(int i)
{
	const char *s;
...
char *DeScumm::get_var_or_word(char *buf, char condition)
{
	if (condition)
		get_var(buf);
	else
...
char *DeScumm::get_var_or_byte(char *buf, char condition)
{
	if (condition)
		get_var(buf);
	else
...
char *DeScumm::get_var_until_0xff(char *buf)
{
	int i;
	int j = 0;
	bool first = true;
...
char *DeScumm::putascii(char *buf, int i)
{
	if (i > 31 && i < 128) {
		buf[0] = i;
		buf[1] = 0;
...
char *DeScumm::get_ascii(char *buf)
{
	int i;
...
char *DeScumm::do_tok(char *buf, const char *text, int args)
{
	char *buforg = buf;
...
void DeScumm::do_decodeparsestring_v2(char *buf, byte opcode)
{
	byte c;
	bool flag;
...
void DeScumm::do_actorset_v12(char *buf, byte opcode)
{
...
void DeScumm::do_actorset(char *buf, byte opcode)
{
...
void DeScumm::AddToExprStack(char *s)
{
	int l;
	char *m;
...
char *DeScumm::GetFromExprStack(char *buf)
{
	char *s;
...
void DeScumm::do_expr_code(char *buf)
{
	int i;
	char *s;
	char *buf2;
...
void DeScumm::do_load_code_to_string(char *buf, byte opcode)
{
...
void DeScumm::do_resource_v2(char *buf, byte opcode)
{
	const char *resTypes[] = {
			"UnkResType0",
			"UnkResType1",
...
void DeScumm::do_resource(char *buf, byte opco)
{
	// FIXME:
...
void DeScumm::do_pseudoRoom(char *buf)
{
	int j, i = get_byte();
...
void DeScumm::do_room_ops(char *buf)
{
	int opcode = get_byte();
...
void DeScumm::do_room_ops_old(char *buf, byte master_opcode)
{
...
void DeScumm::do_cursor_command(char *buf)
{
	int opcode = get_byte();
...
void DeScumm::do_verbops_v2(char *buf, byte opcode)
{
	int subop = get_byte();
...
void DeScumm::do_verbops(char *buf, byte opcode)
{
	char first = 1;
...
void DeScumm::do_print_ego(char *buf, byte opcode)
{
	int i;
...
void DeScumm::do_unconditional_jump(char *buf)
{
...
void DeScumm::emit_if(char *buf, char *condition)
{
	int offset = get_signed_word();
	int cur = get_curoffs();
	int to = cur + offset;
...
void DeScumm::do_if_code(char *buf, byte opcode)
{
	char var[256];
...
void DeScumm::do_if_state_code(char *buf, byte opcode)
{
	char var[256];
	char tmp[256], tmp2[256];
	byte neg;
...
void DeScumm::do_varset_code(char *buf, byte opcode)
{
	char *s;
...
void DeScumm::do_matrix_ops(char *buf, byte opcode)
{
	opcode = get_byte();
...
void DeScumm::get_tok_V12(char *buf)
{
...
void DeScumm::get_tok_V345(char *buf)
{
	byte opcode = get_byte();
...
int DeScumm::skipVerbHeader_V12(byte *p)
{
	byte code;
	int offset = 15;
	int minOffset = 255;
...
int DeScumm::skipVerbHeader_V34(byte *p)
{
	byte code;
...
byte *DeScumm::skipVerbHeader_V5(byte *p)
{
	byte code;
...
char *DeScumm::strecpy(char *buf, const char *src)
{
	strcpy(buf, src);
	return strchr(buf, 0);
}

int DeScumm::get_curoffs()
{
	return cur_pos - org_pos;
}

int DeScumm::get_byte()
{
	return (byte)(*cur_pos++);
}

uint DeScumm::get_word()
{
	int i;
...
char *DeScumm::getIndentString(int level)
{
	if (level >= 127)
		level = 127;
	if (level < 0)
...
void DeScumm::outputLine(char *buf, int curoffs, int opcode, int indent)
{
	char *s;
	//char buffer[2560];
...
bool DeScumm::indentBlock(unsigned int cur)
{
	BlockStack *p;
...
BlockStack *DeScumm::pushBlockStackItem()
{
	if (!block_stack)
...
bool DeScumm::maybeAddIf(uint cur, uint to)
{
	int i;
	BlockStack *p;
...
bool DeScumm::maybeAddElse(uint cur, uint to)
{
	BlockStack *p;
...
bool DeScumm::maybeAddElseIf(uint cur, uint elseto, uint to)
{
	uint k;
	BlockStack *p;
